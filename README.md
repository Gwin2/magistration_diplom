...
При первом запуске вход выполняется как `admin` / `admin123`. Администратор может открыть раздел `Пользователи` в боковом меню, добавить аккаунты, назначить роль, изменить пароль или удалить пользователя. Данные аккаунтов хранятся в `workspace_state/users.json`; начальные параметры можно задать через `UAV_CONTROL_ADMIN_USERNAME` и `UAV_CONTROL_ADMIN_PASSWORD`.
